...
function createEnvironment (dbName, memory, heap, malloc, options = {}) {
  ...
  const { cache, totalPages } = options
  let dbSize = options.dbSize || 0
  ...
  const environment = {
    ...
    wasm_console_log: (i0, i1) => {
      //postMessage({ errorMessage: `code=${i0} msg=${ReadCString(heap, i1)}` })
    },
...
from 
      ...
      for (const name of Object.keys(db.databases)) {
        ...
        for (const key of keys) {
          ...
          if (parts && parts.length > 2) {
            ...
            if (end - start + 1 === 4096) {
              //pages.push({ start, end, size: (end - start) })
  ...
  const { instance } = await WebAssembly.instantiateStreaming(await fetch(wasmPath), imports)
